...
class BaseDao {
  ...
  /**
    * 执行事务性的增、删、改操作
    *
    * @param connection 需要传入连接 Connection
    * @param sql        传入需要执行的SQL
    * @param args       SQL中对应的变量部分
    * @return 返回执行成功的受影响行数
    */
  def update(connection: Connection, sql: String, args: Any*): Int = {
    val i = JdbcUtils.executeUpdate(connection, sql, args.asInstanceOf[util.List[AnyRef]])
